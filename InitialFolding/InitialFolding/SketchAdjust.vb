...
Public Class SketchAdjust
        ...
        Try
            ...
            Else
                ...
                Else
                    If IsLastAngleOk(angle) Then
                        angle.Driven = True
                        If AdjustDimensionConstraint3DSmothly(gap, gap.Parameter._Value * 0.9) Then
                            angle.Driven = False
                            AdjustGapSmothly(gap, setpoint, angle)
                        End If
    ...
    Function AdjustResolution(name As String, s As Double) As Double
        ...
        dc = GetDimension(name)
        If dc.Type = ObjectTypeEnum.kTwoLineAngleDimConstraint3DObject Then
            c = s * dc.Parameter._Value * 64
            r = (1 / (s + Math.Pow(10, -6)) + resolution - 1) / ((counter / 32) + 1)
            If r < 32 Then
                resolution = 32
            Else
                resolution = r
